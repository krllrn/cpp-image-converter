...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    * VERY IMPORTANT: use "b" option to _wfopen() if you are on a machine that
    ...
    */
    int row_stride = image.GetWidth() * 3; /* JSAMPLEs per row in image_buffer */
    ifstream ifs(file, ios::binary);
    std::vector<JSAMPLE> buff_j(row_stride);
    ...
    for (int x = 0; x < image.GetWidth(); ++x) {
        buff_j[x * 3 + 0] = static_cast<JSAMPLE>(line[x].r);
        buff_j[x * 3 + 1] = static_cast<JSAMPLE>(line[x].g);
        buff_j[x * 3 + 2] = static_cast<JSAMPLE>(line[x].b);
    }

    auto data_j = buff_j.data();
    auto ptr = &data_j;
...
Image LoadJPEG(const Path& file) {
    ...
    // нестандартную функцию _w_wfopen
#ifdef _MSC_VER
